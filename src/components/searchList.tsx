...
const SearchList: React.FC<SearchListProps> = ({ coins }) => {
  const [favoritesCoins, setFavoritesCoins] = useState<string[]>(() => {
    const savedFavorites = localStorage.getItem('favoritesCoins');
  ...
  return (
    <div className='list_container'>
      ...
      <div className="search_container">
        ...
        {searchText && (
          <svg onClick={() => setSearchText('')} xmlns="http://www.w3.org/2000/svg" width="1.2em" height="1.2em" viewBox="0 0 40 40" style={{cursor: 'pointer'}}>
      ...
      <div className='topics'>
        <button 
          ...
          onClick={handleFavoritesCoins} 
          style={displayFavorites ? {cursor: 'default', background: '#4c4c4c', borderRadius: '5px'} : {cursor: 'pointer'}}
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="2em" height="2em" viewBox="0 0 24 24">
        ...
        <button 
          className="allCoins_btn" 
          onClick={handleAllCoins}
          style={!displayFavorites ? {cursor: 'default', background: '#4c4c4c', borderRadius: '5px'} : {cursor: 'pointer'}}
        >
            All coins
        </button>
      </div>
      {displayFavorites && favoritesCoins.length === 0 ? (
        <p style={{ color: '#fff' }}>You haven't any favorites coin</p>
      ) : (
          ...
          <VirtualScroll
            ...
            renderItem={(index: number) => (
              <li key={index} className="coin_item">
                <svg
                  onClick={() => toggleToFavorites(filteredCoins[index])}
                  xmlns="http://www.w3.org/2000/svg"
                  width="1.5em"
                  height="1.5em"
                  viewBox="0 0 24 24"
                >
                  {!favoritesCoins.includes(filteredCoins[index]) ? (
                    <path
                      fill="#828282"
                    ...
                    <path
                      fill="#828282"
                      d="M 7.325 18.923 L 8.565 13.61 L 4.442 10.038 L 9.873 9.568 L 12 4.557 L 14.127 9.567 L 19.557 10.037 L 15.434 13.609 L 16.675 18.922 L 12 16.102 L 7.325 18.923 Z M 12 12.25"
                    ></path>
                  )}
                </svg>
                {filteredCoins[index]}
